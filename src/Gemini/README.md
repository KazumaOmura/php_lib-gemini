...
Google Gemini API のクライアント群。テキスト生成、JSON生成、**音声生成 (TTS)**、マルチモーダル入力、バッチ処理、File API、Vertex AI バッチ（GCS）に対応。

## 提供クライアント

| クラス | 用途 |
|---|---|
| `GeminiClient` | リアルタイムの `generateContent` 呼び出し。テキスト・JSON・音声・マルチモーダル入力に対応 |
...
> `AiModel::isTts()` で TTSモデルかどうかを判定できます。

## 用途別のショートカット

`GeminiClient` には用途別の便利メソッドがあります。複雑な引数を覚えずに済むため、こちらを使うのが推奨です。

| メソッド | 用途 | 戻り値 |
|---|---|---|
| `generateText($prompt, $inline_data = [])` | フリーテキスト生成 | `ResponseDto` |
| `generateJson($prompt, $inline_data = [])` | JSON生成 (`responseMimeType=application/json`) | `ResponseDto` |
| `generateAudio($prompt, $voice = Voice::KORE)` | 音声生成 (TTS) | `AudioResponseDto` |
| `generateMultiSpeakerAudio($prompt, $speakers)` | 複数話者の対話音声 | `AudioResponseDto` |
| `request($prompt, $inline_data, $is_batch, $json_mode)` | 低レベルAPI（フル制御） | `ResponseDto \| BatchResponseDto` |

---

## ユースケース1: テキスト生成（フリーテキスト）
...
$response = $client->generateText('俳句を3つ作ってください。');
...
## ユースケース2: 音声生成 (Text-to-Speech)

`generateAudio()` でテキストを Gemini TTS に読み上げさせる。返ってくるのは 24kHz / 16-bit モノラルの PCM。
...
// コンストラクタはテキスト用モデルでOK。generateAudio() の中で TTS モデルに自動切り替え。
$client = new GeminiClient(getenv('GEMINI_API_KEY'), AiModel::GEMINI_2_5_FLASH);

$response = $client->generateAudio(
    prompt: 'Say cheerfully: 良い一日をお過ごしください！',
    voice:  Voice::KORE,
);

// WAVヘッダ付きで保存（ブラウザや一般プレイヤーで再生可能）
$response->saveAsWav(__DIR__ . '/greeting.wav');

// プレーンPCMバイトとして使う
$pcm = $response->getAudioBytes();
echo "MIME: " . $response->getAudioMimeType() . PHP_EOL;
...
```php
// コンストラクタでTTSモデルを直接指定
$client = new GeminiClient(getenv('GEMINI_API_KEY'), AiModel::GEMINI_2_5_PRO_TTS);
$client->generateAudio('Hello', Voice::PUCK);

// または、generateAudio の第3引数でメソッド単位で上書き
$client->generateAudio('Hello', Voice::PUCK, AiModel::GEMINI_3_1_FLASH_TTS);
```

### 利用可能な Voice

30種類のプリセット音声から選べます（`Voice` enum）。代表的なもの:

| 値 | キャラクター |
|---|---|
| `Voice::KORE` | 落ち着き・標準 |
| `Voice::PUCK` | 明るく軽快 |
| `Voice::CHARON` | 深い・落ち着いた男性 |
| `Voice::AOEDE` | 朗らかな女性 |
| `Voice::FENRIR` | 力強い男性 |
| `Voice::LEDA` | 優しい女性 |

全リストは `src/Gemini/Enums/Voice.php` を参照。

### 複数話者の対話を生成

`generateMultiSpeakerAudio()` で、話者ごとに音声を切り替えた対話音声を1リクエストで作れる。

```php
$response = $client->generateMultiSpeakerAudio(
    prompt: <<<TXT
...
$response->saveAsWav(__DIR__ . '/dialog.wav');
```

「Say cheerfully:」「Whisper:」のようなスタイル指示をプロンプト先頭に書くと、声の調子を変えてくれる。

---

## ユースケース3: 画像入力（マルチモーダル）
...
20MBを超える画像/PDF/動画を扱う場合は [File API](#ユースケース6-file-apiでのファイルアップロード) と `file_uri` を使用。

---

## ユースケース4: Safety Settings（有害コンテンツのフィルタ調整）
...
## ユースケース5: Thinking モード（深い推論を必要とするタスク）
...
## ユースケース8: Vertex AI バッチ（GCS 経由・大規模向け）
